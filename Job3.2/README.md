#Comandi per riprodurre gli esperimenti eseguiti rigurado il job 3.2. Con wsl sono stati eseguiti gli esperimenti riguardanti il dataset ridotto e completo, di cui vi sono 2 versioni degli script dove cambia solo l'input.
...
time mapred streaming \
  ...
  -output file://$(pwd)/output_mr_local_job3_2
...
time hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-3.4.1.jar \
  ...
  -mapper "python3 mapper.py" \
  -reducer "python3 reducer.py" \
  -input /user/hadoop/job3.2/input/NOME_DATASET.csv \
  -output /user/hadoop/job3.2/output_mr \
  -numReduceTasks 3

#Hive esecuzione locale 
time hive   -hiveconf mapreduce.framework.name=local   -f job3_2.hql
...
time spark-submit --master local[*] job3_2_spark.py
...
time spark-submit --master local[*] job3_2_sparksql.py
...
time hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
  -files s3://nome_bucket_s3/scripts/job3.2/mapper.py,s3://nome_bucket_s3/scripts/job3.2/reducer.py \
  -mapper "python3 mapper.py" \
  -reducer "python3 reducer.py" \
  -input s3://nome_bucket_s3/input/job3.2/ \
  -output s3://nome_bucket_s3/output/job3_2_mr/

#Hive
time hive -f s3://nome_bucket_s3/scripts/job3.2/job3_2AWS.hql

#SparkCore locale
aws s3 cp s3://nome_bucket_s3/scripts/job3.2/job3_2_sparkAWS.py 
time spark-submit \
  --master "local[*]" \
  job3_2_sparkAWS.py

#SparkCore
time spark-submit s3://nome_bucket_s3/scripts/job3.2/job3_2_sparkAWS.py

#SparkSQ locale
aws s3 cp s3://nome_bucket_s3/scripts/job3.2/job3_2_sparksqlAWS.py 
time spark-submit \
  --master "local[*]" \
  job3_2_sparksqlAWS.py

#SparkSQL
time spark-submit s3://nome_bucket_s3/scripts/job3.2/job3_2_sparksqlAWS.py

#Comandi utili alla riuscita degli esperimenti

#Conversione di mapper e reducer in script unix
dos2unix mapper.py reducer.py

#Lettura delle prime 20 righe dell'output 
hdfs dfs -cat /user/hadoop/job3.2/output/part-* | head -n 20

#Rimozione dell'output per esecuzioni in serie
hdfs dfs -rm -r /user/hadoop/job3.2/output 2>/dev/null
...
hdfs dfs -rm -f /user/hadoop/job3.2/input_ridotto/dataset_job_3_2_ridotto.csv
hdfs dfs -put dataset_job_3_2_ridotto.csv /user/hadoop/job3.2/input_ridotto/
