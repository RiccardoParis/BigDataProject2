...
if __name__ == "__main__":
    ...
    # 1. Lettura Dati
    df = spark.read.csv("/user/hadoop/job3.2/input_ridotto/dataset_job_3_2_ridotto.csv", header=True, inferSchema=True)
    ...
    final_df.rdd.map(lambda row: "\t".join([str(c) for c in row])).saveAsTextFile("/user/hadoop/job3.2/output_sparksql_ridotto")